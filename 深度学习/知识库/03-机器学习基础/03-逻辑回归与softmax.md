...
$K$ 个线性打分把空间切成 $K$ 块,两类得分相等处 $\mathbf w_i\cdot\mathbf x=\mathbf w_j\cdot\mathbf x$ 是一条**线性边界**。所以逻辑回归、softmax 都是**线性分类器**,决策边界是直的;要拟合弯曲边界,必须引入非线性——这正是通往 [多层感知机](node:mlp) 的动因。
...
- 逻辑回归 / softmax 都是**线性分类器**,边界是直的——弯曲边界需非线性(深度网络),见多层感知机。
